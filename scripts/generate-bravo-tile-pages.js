#!/usr/bin/env node
/**
 * Generate individual tile pages for Bravo Tile products
 */
...
const bravoTiles = data.tiles.filter(t => t.brand === 'bravo-tile');

console.log(`Found ${bravoTiles.length} Bravo Tile products`);

// Generate page template
function generatePage(tile) {
  const title = tile.name;
  const slug = tile.slug;
  const image = tile.primaryImage || '/images/placeholder-tile.jpg';
  const material = tile.material || '';
  const style = tile.type && tile.type !== 'Tile' ? tile.type : '';
  const category = tile.category || '';
  const color = tile.primaryColor || '';

  // Build description parts
  let descParts = [];
  if (category) descParts.push(category);
  if (material && material !== category) descParts.push(material);
  if (style) descParts.push(style.toLowerCase());

  const typeDesc = descParts.length > 0 ? descParts.join(' ') + ' tile' : 'premium tile';
  const description = `${title} from Bravo Tile - ${typeDesc.charAt(0).toUpperCase() + typeDesc.slice(1)}. Available at Surprise Granite for your kitchen and bathroom remodeling projects in the Phoenix metro area.`;
...
<head>
  ...
  <title>${title} | Bravo Tile | Surprise Granite AZ</title>
  <meta name="description" content="${description}"/>
  <link rel="canonical" href="https://surprisegranite.com/tiles/${slug}/"/>

  <meta property="og:type" content="product"/>
  <meta property="og:url" content="https://surprisegranite.com/tiles/${slug}/"/>
  <meta property="og:title" content="${title} | Bravo Tile"/>
...
<script defer src="/js/unified-nav.js?v=20260712b"></script>
...
<body>
  ...
  <div class="breadcrumb-bar">
    <nav class="breadcrumb">
      ...
      <a href="/materials/all-tile">Tile</a>
      <span>&rsaquo;</span>
      <a href="/vendors/bravo-tile">Bravo Tile</a>
      <span>&rsaquo;</span>
      <strong>${title}</strong>
    </nav>
  </div>
    ...
    <div class="product-layout">
      <div class="gallery">
        <div class="main-image-container">
          <img src="${image}" alt="${title}" class="main-image" onerror="this.src='/images/placeholder-tile.jpg'"/>
          <span class="image-badge">${style}</span>
        </div>
      </div>

      <div class="product-info">
        <div class="brand-row">
          <span class="brand-name">Bravo Tile</span>
        </div>

        <h1 class="product-title">${title}</h1>
        <p class="product-subtitle">Premium ${material} ${style}</p>

        <div class="product-specs">
          ${material ? `<span class="spec-tag">${material}</span>` : ''}
          ${style ? `<span class="spec-tag">${style}</span>` : ''}
          ${color ? `<span class="spec-tag">${color}</span>` : ''}
        </div>
...
bravoTiles.forEach(tile => {
  const dir = path.join(TILES_DIR, tile.slug);
  ...
  const html = generatePage(tile);
  fs.writeFileSync(file, html);
  created++;

  console.log(`Created: /tiles/${tile.slug}/`);
});

console.log(`\nDone! Created ${created} tile pages.`);
