#!/usr/bin/env python3
"""
Fill-only writer (any vendor): append a proposal's images to the live Supabase
catalog_products.image_urls.

SAFETY:
  * Reads the CURRENT image_urls from the DB and APPENDS only — never removes.
  * Dedups by asset id (Widen /content/<id>/ or Scene7 /is/image/<v>/<id>).
  * Leaves primary_image_url untouched (the slab stays the grid thumbnail;
    lifestyle/room shots become secondary gallery images).
  * Backs up every affected row's image_urls to ~/sg-backups/<vendor>-... BEFORE
    writing. Reverse = restore image_urls from it.
  * --dry-run (default) writes nothing; --write commits in one transaction.

Usage:
  python3 write-vendor-images.py --vendor daltile --proposal daltile-proposal.json
  python3 write-vendor-images.py --vendor daltile --proposal daltile-proposal.json --write
"""
...
def asset_key(url):
    u = url or ''
    m = re.search(r'/content/([a-z0-9]+)/', u)          # Widen
    if m: return m.group(1)
    m = re.search(r'/is/image/[^/]+/([^/?]+)', u)         # Scene7
    if m: return re.sub(r'_(zoom_\d+|\d{3,4}|32|v\d+|web\d?|CU\d+)', '', m.group(1), flags=re.I).lower()
    return u.split('?')[0]

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument('--vendor', required=True, help="catalog_products.vendor_id, e.g. daltile")
    ap.add_argument('--proposal', required=True, help="proposal JSON filename in scratchpad")
    ap.add_argument('--write', action='store_true', help='commit changes (default is dry run)')
    args = ap.parse_args()
    VENDOR = args.vendor
    proposal_path = args.proposal if '/' in args.proposal else str(SCRATCH / args.proposal)
    ...
    rows = psql_json(
        ...
        f"where vendor_id='{VENDOR}' and category='slab' and slug in ({inlist});")
    db = {r['slug']: r for r in rows}
    print(f"vendor={VENDOR} | proposal slugs: {len(slugs)} | found in DB: {len(db)}")
    ...
    backup = BACKUP_DIR / f"{VENDOR}-image-urls-backup.tsv"
    ...
    for slug, old, new, n in plans:
        ...
        stmts.append(
            ...
            f"WHERE vendor_id='{VENDOR}' AND category='slab' AND slug='{s}';")
    stmts.append("COMMIT;")
    sqlfile = SCRATCH / f"{VENDOR}-image-write.sql"
    sqlfile.write_text("\n".join(stmts))
