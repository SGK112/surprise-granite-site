#!/usr/bin/env python3
"""
Replace Webflow navbar with secondary header in tools pages.
"""
...
.header-nav a:hover { color: #cca600; }
.btn-estimate {
  background: #cca600 !important;
...
def update_page(filepath):
    ...
    try:
        ...
        if 'class="header">' in content and 'class="header-content">' in content:
            print(f"  SKIP (already updated): {filepath}")
            return False

        # Remove Webflow navbar - pattern: <div...class="navbar-fixed_wrapper">...</div> followed by content
        # This is tricky because the navbar can be very long

        # First, try to remove navbar-fixed_wrapper div
        # Pattern matches from <div...navbar-fixed_wrapper> to the closing that precedes main content
        navbar_pattern = r'<div[^>]*class="navbar-fixed_wrapper"[^>]*>.*?</nav>\s*</div>\s*</div>'
        content = re.sub(navbar_pattern, '', content, flags=re.DOTALL)

        # Also try alternate pattern
        navbar_pattern2 = r'<div data-navbar-height[^>]*class="navbar-fixed_wrapper"[^>]*>.*?</nav>\s*</div>\s*</div>'
        content = re.sub(navbar_pattern2, '', content, flags=re.DOTALL)

        # Add mobile-optimizations.css if not present
        if 'mobile-optimizations.css' not in content:
            content = content.replace('</head>', f'  {CSS_LINK}\n</head>')

        # Add header CSS if not present
        if '.header-content {' not in content:
            # Find first </style> and add our CSS before it
            content = re.sub(r'(</style>)', HEADER_CSS + r'\1', content, count=1)
        ...
        if re.search(body_pattern, content):
            ...
            if '<div class="page-wrapper">' in content:
                # Insert after page-wrapper and global-styles
                content = re.sub(
                    r'(<div class="page-wrapper">.*?</div>)',
                    r'\1' + SECONDARY_HEADER,
                    content,
                    count=1,
                    flags=re.DOTALL
                )
                ...
                content = re.sub(body_pattern, r'\1' + SECONDARY_HEADER, content, count=1)
...
def main():
    ...
    print("=== Updating Tools Pages with Secondary Header ===\n")

    for page_name in TOOLS_PAGES:
        page_path = tools_dir / page_name / "index.html"
        if page_path.exists():
            result = update_page(page_path)
            if result:
                updated += 1
            else:
                skipped += 1
