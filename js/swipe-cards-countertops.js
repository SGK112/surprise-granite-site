/**
 * Swipe Cards - Countertops
 * "Swipe Right for Your Perfect Countertop!"
...
(function() {
  ...
  async function init() {
    ...
    // Get all product items - only direct children with product-thumb_item
    const items = materialsList.querySelectorAll(':scope > .w-dyn-item');
    if (items.length === 0) return;

    console.log('Swipe Cards: Found', items.length, 'countertop items');

    // Filter to only items that have the product structure
    const productItems = Array.from(items).filter(item =>
      item.querySelector('.product-thumb_item') !== null
    );

    console.log('Swipe Cards: Filtered to', productItems.length, 'actual products');
  ...
  function showIntroOverlay() {
    ...
    overlay.innerHTML = `
      <div class="swipe-intro-content">
        <h2 class="swipe-intro-title">find your countertop</h2>
  ...
  async function loadFavorites() {
    ...
    try {
      const stored = localStorage.getItem('sg_countertop_favorites');
      if (stored) {
  ...
  function saveToLocalStorage() {
    try {
      localStorage.setItem('sg_countertop_favorites', JSON.stringify(favorites));
    } catch (e) {
    ...
    const swipeHTML = `
      <div class="swipe-cards-container">
        <div class="swipe-topbar">
          <span class="swipe-topbar-title">find your countertop</span>
      ...
      <div class="favorites-drawer" id="favorites-drawer">
        ...
        <div class="favorites-actions">
          ...
          ${favorites.length > 0 ? `<a href="/account/#saved-countertops" class="favorites-view-all">View All</a>` : ''}
        </div>
      </div>
    `;
        ...
        <div class="swipe-card-specs">
          ...
          ${card.brand ? `
            <div class="swipe-card-spec">
              ...
              <span class="spec-value">${card.brand}</span>
            </div>
          ` : ''}
          ${card.style ? `
            <div class="swipe-card-spec">
              <span class="spec-label">Style</span>
              <span class="spec-value">${card.style}</span>
            </div>
          ` : ''}
        </div>
      </div>
  ...
  function updateFavorites() {
    ...
    if (listEl) {
      if (favorites.length === 0) {
        listEl.innerHTML = '<div class="favorites-empty">No favorites yet. Swipe right to save countertops!</div>';
      } else {
  ...
  function showEmptyState() {
    ...
    const accountLink = currentUser
      ? `<a href="/account/#saved-countertops" class="swipe-view-saved-btn">View Saved Countertops</a>`
      : `<a href="/account/" class="swipe-view-saved-btn">Login to Save Favorites</a>`;

    stack.innerHTML = `
      <div class="swipe-empty">
        <div class="swipe-empty-icon">&#127881;</div>
        <h3>You've seen all ${cards.length} countertops!</h3>
