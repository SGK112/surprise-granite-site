...
-- Migration 004: Database Functions and Stored Procedures
-- =============================================

-- =============================================
-- FUNCTION: Get product with type-specific data
-- =============================================
...
BEGIN
  ...
  CASE product_record.product_type
    WHEN 'slab' THEN
      ...
      FROM product_slabs ps WHERE ps.product_id = p_product_id;
    WHEN 'tile' THEN
      SELECT to_jsonb(pt.*) INTO type_data
      FROM product_tiles pt WHERE pt.product_id = p_product_id;
    WHEN 'flooring' THEN
      SELECT to_jsonb(pf.*) INTO type_data
      FROM product_flooring pf WHERE pf.product_id = p_product_id;
    WHEN 'installation_product' THEN
      SELECT to_jsonb(pi.*) INTO type_data
      FROM product_installation pi WHERE pi.product_id = p_product_id;
    ELSE
      type_data := '{}'::JSONB;
  END CASE;
...
BEGIN
  ...
  RETURN QUERY
  SELECT
    get_product_full(p.id) AS product,
...
-- =============================================
-- FUNCTION: Adjust inventory with transaction logging
-- =============================================
...
BEGIN
  ...
  SELECT quantity INTO current_qty FROM products WHERE id = p_product_id FOR UPDATE;
  ...
  LOOP
    BEGIN
      ...
      IF product_id IS NOT NULL THEN
        IF EXISTS (SELECT 1 FROM products WHERE id = product_id AND created_at = updated_at) THEN
...
-- =============================================
-- FUNCTION: Find product by external SKU
-- =============================================
...
BEGIN
  ...
  IF found_product_id IS NOT NULL THEN
    RETURN get_product_full(found_product_id);
  END IF;
...
-- =============================================
-- FUNCTION: Record sync operation
-- =============================================
...
DECLARE
  log_id UUID;
BEGIN
  INSERT INTO sync_logs (integration_id, sync_type, direction, status)
