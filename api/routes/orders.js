...
/**
 * Build one vendor's PO without sending it.
 *
 * Shared by the preview and the send so what staff approve on screen is byte
 * for byte what the vendor receives — a preview generated by different code
 * than the send is a preview of nothing.
 *
 * Returns { error, code } instead of throwing so callers can pick the status.
 */
async function buildVendorPO(supabase, orderId, vendorId, note) {
  ...
  if (['refunded', 'cancelled'].includes(order.status)) {
    return { error: `Order is ${order.status} — not raising a PO`, code: 400 };
  }
  if (order.payment_status && !['paid', 'partially_refunded'].includes(order.payment_status)) {
    return { error: `Order is not paid (${order.payment_status}) — not raising a PO`, code: 400 };
  }

  const items = vendorsOnOrder(order).get(vendorId) || [];
  if (!items.length) return { error: 'No items for that vendor on this order', code: 400 };
  ...
  const poNumber = leg?.po_number
    || 'PO-' + String(order.order_number || orderId).replace(/[^A-Za-z0-9-]/g, '').toUpperCase()
  ...
  return {
    order, leg, items, vendor: vendor || null, poNumber, subject, html,
    to: vendor?.dropship_email || null,
    alreadySent: !!leg?.po_number
  };
}

/**
 * POST /api/admin/orders/:id/fulfillments/:vendorId/po/preview
 *
 * Step one of two. Renders the exact email and says exactly who it would go to.
 * Changes nothing — no PO number is claimed, no status moves, nothing is sent.
 */
router.post('/:id/fulfillments/:vendorId/po/preview', adminAccess, async (req, res) => {
  try {
    const supabase = req.app.get('supabase');
    const { id, vendorId } = req.params;
    const built = await buildVendorPO(supabase, id, vendorId, req.body?.note);
    if (built.error) return res.status(built.code).json({ error: built.error });

    res.json({
      po_number: built.poNumber,
      to: built.to,
      vendor_name: built.vendor?.vendor_name || vendorId,
      subject: built.subject,
      html: built.html,
      item_count: built.items.length,
      already_sent: built.alreadySent,
      // Not a hard error at preview time — staff should SEE the PO and be told
      // where it cannot go, rather than getting a bare refusal with no document.
      blocked: built.to ? null : `No order email on file for "${vendorId}". Add vendor_config.dropship_email before sending.`
    });
  } catch (err) {
    logger.error('Error building PO preview:', err.message);
    res.status(500).json({ error: 'Failed to build the PO' });
  }
});
 ...
 *
 * Step two. Sends the PO previewed above, stamps the number, moves the leg to
 * 'ordered' and rolls the order status up.
 *
 * ⚠️ Deliberately NOT automatic on payment. A PO is a promise to a supplier and
 * cannot be unsent. On 2026-08-27 a Whitehaus sink sold for $105.30 against a
 * $1,550 MSRP because of a bad scraped price; auto-sending would have put a PO
 * in Alfi's inbox before anyone noticed. The human approval is the safeguard.
 * If a vendor is ever trusted enough to skip it, gate that per vendor in
 * vendor_config — never globally.
 */
router.post('/:id/fulfillments/:vendorId/send-po', adminAccess, async (req, res) => {
  try {
    const supabase = req.app.get('supabase');
    const { id, vendorId } = req.params;
    const { note, resend = false } = req.body || {};

    const built = await buildVendorPO(supabase, id, vendorId, note);
    if (built.error) return res.status(built.code).json({ error: built.error });
    ...
    if (built.alreadySent && !resend) {
      return res.status(409).json({
        error: `PO ${built.poNumber} was already sent to ${built.to}. Confirm to send it again.`,
        po_number: built.poNumber
      });
    }

    const result = await emailService.sendNotification(built.to, built.subject, built.html);
    if (!result.success) {
    ...
    await supabase.from('order_fulfillments').update({
      po_number: built.poNumber,
      status: (!built.leg?.status || built.leg.status === 'pending') ? 'ordered' : built.leg.status,
      ordered_at: built.leg?.ordered_at || now,
      updated_at: now
    }).eq('order_id', id).eq('vendor_id', vendorId);
    ...
    await supabase.from('orders')
      .update({ status: rollUpStatus(allLegs || [], built.order.status), updated_at: now })
      .eq('id', id);

    await logOrderEvent(
      supabase, id, 'po_sent',
      `${built.poNumber} emailed to ${built.vendor?.vendor_name || vendorId} <${built.to}> — ${built.items.length} line(s)`,
      req.adminUser?.email
    );

    res.json({ sent: true, po_number: built.poNumber, to: built.to, items: built.items.length });
  } catch (err) {
