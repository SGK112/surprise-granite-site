...
// The Yard charges us 9.1% AZ sales tax on material (we're not tax-exempt with them, order
// #67349). retail_price stays the raw Yard price (the grid + Aria add fab on top); vendor_cost
// = raw * 1.091 so our recorded cost reflects the tax we actually pay.
...
async function syncOnce(supabase, logger) {
  ...
  for (const p of existing) {
    ...
    if (!live && p.active) {
      ...
      const cost = r2(live.Price * YARD_TAX);
      if (Number(p.retail_price) !== live.Price || Number(p.vendor_cost) !== cost) { fields.retail_price = live.Price; fields.sample_price = live.Price; fields.vendor_cost = cost; }
  ...
  for (const [pid, r] of liveById) {
    ...
    const { error } = await supabase.from('catalog_products').insert({
      ...
      retail_price: r.Price, vendor_cost: r2(r.Price * YARD_TAX), sample_price: r.Price, price_unit: 'each',
  ...
  for (const p of slabRows) {
    ...
    if (!c && p.active) {
      ...
      const fields = {};
      if (!p.active) { fields.active = true; fields.in_stock = true; }
      const cost = r2(best.Price * YARD_TAX);
      if (Number(p.retail_price) !== best.Price || Number(p.vendor_cost) !== cost) { fields.retail_price = best.Price; fields.sample_price = best.Price; fields.vendor_cost = cost; }
  ...
  for (const [key, c] of colors) {
    ...
    const { error } = await supabase.from('catalog_products').insert({
      ...
      retail_price: best.Price, vendor_cost: r2(best.Price * YARD_TAX), sample_price: best.Price, price_unit: 'each',
      sample_eligible: false, in_stock: true, active: true,
